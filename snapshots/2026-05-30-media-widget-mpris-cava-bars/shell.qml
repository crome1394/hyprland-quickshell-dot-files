    ...
    QtObject {
        ...
        function refreshPlayers() {
            ...
            for (let i = 0; i < raw.length; i++) {
                ...
                if (!t && p.playbackState === MprisPlaybackState.Stopped) continue;  // completely dead
                if (p.isPlaying) active.push(p);
                else paused.push(p);
            }

            // Prefer any playing streams. Fall back to paused ones that have titles.
            let list = active.length > 0 ? active : paused;

            // Keep current selection stable if possible
            let newIdx = 0;
            if (media.currentPlayer && list.length > 0) {
                for (let i = 0; i < list.length; i++) {
                    if (list[i] === media.currentPlayer) { newIdx = i; break; }
                }
            }
            media.players = list;
