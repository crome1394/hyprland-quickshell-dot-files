...
PanelWindow {
    ...
    anchors.left: true
    anchors.right: true
    implicitHeight: 32   // Reduced for compact layout (was 54 after adding widgets)
    color: "transparent"
    ...
    Rectangle {
        ...
        RowLayout {
            ...
            Rectangle {
                ...
                radius: bar.pillRadius
                color: trayHover.containsMouse ? bar.surface : bar.pillBg
                border.width: 1
    ...
    PopupWindow {
        ...
        anchor.window: bar
        implicitWidth: 310
        implicitHeight: 530   // Increased ~50% for better readability on ultrawide
        visible: false
    ...
    PopupWindow {
        ...
        anchor.window: bar
        implicitWidth: 420
        implicitHeight: 470   // Increased another ~50% (total ~2.2x original) for ultrawide readability
        visible: false
        color: "transparent"
    ...
    PopupWindow {
        ...
        implicitHeight: {
            ...
            const topMargin = 18;            // Increased ~50%
            const topSpacer = 6;             // Increased ~50%
            const headerHeight = 22;
            const outerSpacing = 9;          // Increased ~50%
            const perWindowRow = 38;         // Increased ~50% (was 26) - IconImage + text + vertical padding
            const innerListSpacing = 6;      // Increased for taller feel (was 4)
            const bottomSpacer = 20;         // Increased ~50%
            const bottomMargin = 20;         // Increased ~50%
            const safetyBuffer = 18;         // Increased ~50%
            ...
            // Increased ~50% taller overall (max now ~1140) for better readability on ultrawide
            return Math.max(90, Math.min(1140, total));
        }
                ...
                ColumnLayout {
                    ...
                    Repeater {
                        ...
                        delegate: Rectangle {
                            ...
                            radius: 4
                            color: entryMouse.containsMouse && !modelData.isSeparator ? bar.surface : "transparent"
