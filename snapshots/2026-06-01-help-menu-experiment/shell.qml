...
PanelWindow {
    ...
    // ===== MEDIA PLAYER (MPRIS) =====
    // Centered pill in top bar. Shows only when media is active.
    // Scroll cycles streams. Right-click opens rich popup with art + full controls.
    // Visualizer is pure-QML (no cava binary dependency) — animated bars behind title.
    QtObject {
        id: media
        ...
        // Filtered "interesting" players: prefer actively playing, include paused with titles
        // This handles the user's case of multiple browser tabs (some paused).
        ...
        // Per-player volume (most MPRIS players support this, including browsers)
        readonly property real volume: currentPlayer ? currentPlayer.volume : 0
        readonly property bool volumeSupported: currentPlayer ? !!currentPlayer.volumeSupported : false

        function setVolume(v) {
            if (currentPlayer && currentPlayer.volumeSupported) {
                currentPlayer.volume = Math.max(0, Math.min(1, v));
            }
        }
        ...
        function refreshPlayers() {
            ...
            for (let i = 0; i < raw.length; i++) {
                ...
                if (!t) continue;   // must have a title (your original requirement: show paused browser tabs too)
                result.push(p);
            }

            // Sort: playing streams first (so the "active" one is usually the default)
            result.sort(function(a, b) {
                const aPlay = a.isPlaying ? 1 : 0;
                const bPlay = b.isPlaying ? 1 : 0;
                return bPlay - aPlay;
            });

            // Keep current selection stable if possible
            let newIdx = 0;
            if (media.currentPlayer && result.length > 0) {
                for (let i = 0; i < result.length; i++) {
                    if (result[i] === media.currentPlayer) { newIdx = i; break; }
                }
            }
            media.players = result;
            if (newIdx >= result.length) newIdx = 0;
    ...
    Rectangle {
        ...
        // The cava-like visualizer as background layer (now spans full 600px)
        Loader {
            id: vizLoader
            anchors.fill: parent
            anchors.margins: 4
    ...
    PopupWindow {
        ...
        Rectangle {
            ...
            ColumnLayout {
                ...
                // Header row: app name + close hint
                RowLayout {
                    Layout.fillWidth: true
                    Text {
                        text: media.appName
                ...
                // Volume slider (per-stream, when the player supports it)
                ColumnLayout {
                    Layout.fillWidth: true
                    ...
                    RowLayout {
                        ...
                        Text {
                            text: media.volume > 0 ? "󰕾" : "󰖁"
                            font.pixelSize: 16
                            color: bar.accent
                            anchors.verticalCenter: parent.verticalCenter
                        }

                        // Clickable volume bar (same aesthetic as seek bar)
                        Item {
                            Layout.fillWidth: true
                            Layout.preferredHeight: 18
                            ...
                            Rectangle {
                                width: Math.max(0, Math.min(parent.width, parent.width * media.volume))
                                height: 4
                                radius: 2
                                color: bar.accent
                                anchors.left: parent.left
                            ...
                            WheelHandler {
                                ...
                                onWheel: (e) => {
                                    const d = (e.angleDelta.y > 0) ? 0.05 : -0.05;
                                    media.setVolume(media.volume + d);
                                }
                            }
                        }
                ...
                // Player / stream selector (only when >1 active)
                ColumnLayout {
                    Layout.fillWidth: true
                                ...
                                delegate: Rectangle {
                                    ...
                                    Column {
                                        ...
                                        Row {
                                            ...
                                            Text {
                                                text: modelData.volumeSupported ? "🔊" : "🔇"
                                                font.pixelSize: 10
                                                color: modelData.volumeSupported ? bar.accent : bar.overlay
                                            }
                                        }
                                        Text {
...
// ===== Hyprland Help Menu (ALT + /) =====
// Instantiated at root level so it can be a true screen-centered floating panel.
HelpMenu { id: helpMenu }

// IPC target so hyprland.lua can do: qs ipc call help toggle
Io.IpcHandler {
    target: "help"
    onSignalTriggered: function (sig, val) {
        if (sig === "toggle" && helpMenu && helpMenu.toggle) {
            helpMenu.toggle()
        }
    }
}
